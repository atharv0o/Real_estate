...
type Props = {
  id: string;
  /** Used when store is empty (direct navigation) */
...
export function PropertyDetailClient({ id, fallback }: Props) {
  ...
  const data =
    propertyData?.id === id ? propertyData : { ...fallback, id };
  ...
  return (
    <>
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        >
          Trend: {data.trend}
          {typeof data.trendPercent === "number" && (
            <span className="ml-1 opacity-80">({data.trendPercent}%)</span>
          )}
        </span>
      </div>
      ...
      {propertyData?.id !== id && (
        <p className="mt-8 rounded-xl border border-amber-500/20 bg-amber-500/10 px-4 py-3 text-sm text-amber-200/90">
          Showing demo data for this URL. Run a search and fetch details to sync
          live data from your API into the app store.
        </p>
      )}
    </>
  );
