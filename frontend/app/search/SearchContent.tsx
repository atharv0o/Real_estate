...
export default function SearchContent() {
  ...
  const { searchParams, propertyData, loading, setSearchParams, setPropertyData, setLoading } = usePropertyStore();

  useEffect(() => {
    if (params) {
      setSearchParams(params);
      console.debug("SearchContent query params", params);
    }
  }, [params, setSearchParams]);

  useEffect(() => {
    if (coords) {
      console.debug("SearchContent resolved coordinates", coords);
    }
  }, [coords]);
  ...
  if (!params) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-20 text-center">
        <h1 className="font-display text-2xl font-semibold text-white">No search query</h1>
        <p className="mt-2 text-slate-400">Start from the home page and submit the location form.</p>
        <Link href="/" className="mt-6 inline-block rounded-xl bg-sky-500 px-6 py-3 text-sm font-semibold text-slate-950">
          Back to home
        </Link>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 md:py-14">
      <div className="mb-8 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
        <div>
          <h1 className="font-display text-2xl font-bold tracking-tight text-white md:text-3xl">Search results</h1>
          <p className="mt-1 text-sm text-slate-400">Mock geocoding from your query - pin shows the resolved coordinates.</p>
        </div>
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_340px]">
        <div className="space-y-6">
          {showMapSkeleton ? (
            ...
            <Map lat={coords.lat} lng={coords.lng} label={coords.label} className="aspect-[16/9] w-full min-h-[280px]" />
          )}
          ...
          {propertyData && !loading && (
            <>
              ...
              />
              {propertyData.priceHistory && <PriceChart data={propertyData.priceHistory} />}
              <Link href={`/property/${propertyData.id}`} className="inline-block text-sm font-medium text-sky-400 hover:text-sky-300">
      ...
      {searchParams && (
        <details className="mt-10 rounded-xl border border-white/5 bg-slate-900/30 p-4 text-xs text-slate-500">
          ...
          <pre className="mt-2 overflow-x-auto font-mono text-[11px] text-slate-400">{JSON.stringify(searchParams, null, 2)}</pre>
        </details>
      )}
    </div>
  );
