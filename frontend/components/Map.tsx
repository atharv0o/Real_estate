...
/**
 * Map placeholder — swap the inner div for @react-google-maps/api or
 * next/script + google.maps.Map when you add a browser API key.
 */
export type MapProps = {
  lat: number;
  lng: number;
  /** Optional label shown near the pin */
  label?: string;
  className?: string;
};
...
export function Map({ lat, lng, label, className = "" }: MapProps) {
  return (
    <div
      ...
      aria-label={
        label
          ? `Map showing ${label} at ${lat.toFixed(4)}, ${lng.toFixed(4)}`
          : `Map at ${lat.toFixed(4)}, ${lng.toFixed(4)}`
      }
    >
      {/* Mock “satellite” gradient + grid to stand in for Google Maps tiles */}
      <div
      ...
      <div className="absolute bottom-0 left-0 right-0 border-t border-white/10 bg-slate-950/80 p-4 backdrop-blur-md">
        ...
        <p className="mt-1 text-[10px] text-slate-500">
          Google Maps integration: pass lat/lng from Geocoding API; this view is a
          visual placeholder.
        </p>
      </div>
    </div>
